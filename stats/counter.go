...
import (
	...
	"github.com/neko233-com/cache233-go/internal/xsync"
)

// Counter is a goroutine-safe [Recorder] implementation for use by cache233.Cache.
type Counter struct {
	hits           *xsync.Adder
	misses         *xsync.Adder
...
// causes an entry to be loaded and the loading completes successfully (either no error or cache233.ErrNotFound).
...
// causes an entry to be loaded, but the loading function returns an error that is not cache233.ErrNotFound.
