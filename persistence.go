...
func LoadCacheFromFile[K comparable, V any](c *Cache[K, V], filePath string) error {
	...
	if err != nil {
		return fmt.Errorf("cache233: open file %s: %w", filePath, err)
	}
...
func LoadCacheFrom[K comparable, V any](c *Cache[K, V], r io.Reader) error {
	...
	if err := dec.Decode(&savedMaximum); err != nil {
		return fmt.Errorf("cache233: decode maximum: %w", err)
	}
	...
	for size < maximum {
		...
		if err := dec.Decode(&entry); err != nil {
			...
			return fmt.Errorf("cache233: decode entry: %w", err)
		}
...
func SaveCacheToFile[K comparable, V any](c *Cache[K, V], filePath string) error {
	...
	if _, err := os.Stat(dir); err != nil {
		if !os.IsNotExist(err) {
			return fmt.Errorf("cache233: stat %s: %w", dir, err)
		}
		if err := os.MkdirAll(dir, 0o755); err != nil {
			return fmt.Errorf("cache233: create dir %s: %w", dir, err)
		}
	}

	file, err := os.Create(filePath)
	if err != nil {
		return fmt.Errorf("cache233: create file %s: %w", filePath, err)
	}
	//nolint:errcheck // it's ok
	defer file.Close()
...
func SaveCacheTo[K comparable, V any](c *Cache[K, V], w io.Writer) error {
	...
	if err := enc.Encode(maximum); err != nil {
		return fmt.Errorf("cache233: encode maximum: %w", err)
	}
	...
	for entry := range c.Hottest() {
		...
		if err := enc.Encode(entry); err != nil {
			return fmt.Errorf("cache233: encode entry: %w", err)
		}
