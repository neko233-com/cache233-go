...
import (
	...
	"github.com/neko233-com/cache233-go/internal/xmath"
)

// Stats are statistics about the performance of an cache233.Cache.
type Stats struct {
	// Hits is the number of times cache233.Cache lookup methods returned a cached value.
	Hits uint64
	// Misses is the number of times cache233.Cache lookup methods did not find a cached value.
	Misses uint64
	// Evictions is the number of times an entry has been evicted. This count does not include manual
	// cache233.Cache deletions.
	Evictions uint64
	// EvictionWeight is the sum of weights of evicted entries. This total does not include manual
	// cache233.Cache deletions.
	EvictionWeight uint64
	// LoadSuccesses is the number of times cache233.Cache lookup methods have successfully loaded a new value.
	LoadSuccesses uint64
	// LoadFailures is the number of times cache233.Cache lookup methods failed to load a new value, either
...
// Requests returns the number of times cache233.Cache lookup methods were looking for a cached value.
//
...
// Loads returns the total number of times that cache233.Cache lookup methods attempted to load new values.
//
// NOTE: the values of the metrics are undefined in case of overflow. If you require specific handling, we recommend
// implementing your own [Recorder].
